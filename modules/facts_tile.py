...
def setup_facts_tile(parent, url, scr, max_items=5):

    """
    Fetch list of facts (JSON array), display scrollable cards.
    """
    def _clean():
    ...
    def _update(timer=None):
        ...
        try:
            r = urequests.get(url)
            facts = ujson.loads(r.text)[:max_items]; r.close()
        except:
            facts = ["Error loading facts"]
